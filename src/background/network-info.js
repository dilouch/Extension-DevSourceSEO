(() => {
	...
	async function update(details) {
		// Ne traiter que la requête principale (main frame)
		...
		await new Promise((resolve) => {
			chrome.storage.local.get([STORAGE_KEY], (result) => {
				const current = result?.[STORAGE_KEY] && typeof result[STORAGE_KEY] === 'object' ? result[STORAGE_KEY] : {};
				// Stocker à la fois par URL ET par tabId pour la robustesse
	...
	async function get(url, tabId) {
		return new Promise((resolve) => {
			chrome.storage.local.get([STORAGE_KEY], (result) => {
				const store = result?.[STORAGE_KEY] && typeof result[STORAGE_KEY] === 'object' ? result[STORAGE_KEY] : {};
				// 1) clé URL exacte
				if (store[url]) return resolve(store[url]);
				// 2) clé par tabId
				if (typeof tabId === 'number' && store['tab:' + tabId]) {
					return resolve(store['tab:' + tabId]);
				}
				// 3) recherche tolérante (sans hash, sans slash final)
				try {
					...
					// 4) fallback : même hôte, entrée la plus récente
					let best = null;
					for (const key of Object.keys(store)) {
						if (key.startsWith('tab:')) continue;
						try {
	...
	globalThis.NetworkInfoV2 = Object.freeze({
		update,
		get
	});
})();
